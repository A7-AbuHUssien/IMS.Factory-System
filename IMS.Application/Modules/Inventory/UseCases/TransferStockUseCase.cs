...
public class TransferStockUseCase : BaseStockUseCase
{
    ...
    public async Task Execute(TransferStockDto dto)
    {
        ...
        try
        {
            ...
            var outTrans = _factory.CreateOut(dto.ProductId, dto.SourceWarehouseId, dto.Quantity, from.AvgCost, 0, from.Quantity,
                "Transfer-Out",TransactionSource.Transfer);
            var inTrans = _factory.CreateIn(dto.ProductId, dto.DestinationWarehouseId, dto.Quantity, from.AvgCost, to.Quantity,
